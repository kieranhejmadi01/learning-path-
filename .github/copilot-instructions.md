...
**Start the Hugo dev server with drafts enabled:**
```bash
./hugo-server.sh
```
This runs Hugo with `--buildDrafts` flag to render draft content. Site is served at `http://localhost:1313/`.

### Validation Pipeline

The project uses Python tools in `/tools/` to validate content integrity before publishing:

- **`verify_index_fields.py`**: Validates YAML frontmatter in all `_index.md` files against required fields, metadata constraints, and closed-list allowances defined in `closed-filters-allow-list.yml`
  - Enforces: title, minutes_to_complete, who_is_this_for, learning_objectives, prerequisites, author, skilllevels, subjects, armips, tools_software_languages, operatingsystems, further_reading, weight, layout, learning_path_main_page
  - Validates skill levels are only "Introductory" or "Advanced"
  - Validates subjects/OS against allowlist per category (category rules in `closed-filters-allow-list.yml`)
  
- **`style_check.py`**: Lints markdown against writing style guidelines from `style_rules.json`
  - Detects passive voice using spaCy NLP
  - Enforces phrase preferences (e.g., "use" vs. "utilize", "because" vs. "since", contractions)
  - Skips content in code blocks and YAML frontmatter
  - Outputs suggestions to `data/style_suggestions.json`
  
- **`check.py`**: Orchestrates test execution for code blocks embedded in Learning Paths using Docker containers
  - Parses markdown for fenced code blocks with test metadata
  - Executes commands in isolated containers and validates output
  - Generates JUnit-compatible test reports

**Run all validation before committing:**
```bash
python3 tools/verify_index_fields.py    # Check metadata
python3 tools/style_check.py <filepath> # Check writing style
./tools/test_lp.sh                      # Run embedded code tests (if applicable)
```

### Content Structure Patterns

**Learning Path file organization:**
- `_index.md`: Main content. Must include weight=1 and learning_path_main_page=yes
- `_next-steps.md`: Fixed template for related links (modify only the content area)
- `how-to-*.md`: Additional instructional files (weight=2,3, etc. for ordering)

**Install Guide file organization:**
- `_index.md`: Overview page (weight=1)
- Additional `.md` files for multi-part guides (set multitool_install_part=true)

**All Learning Path markdown files must start with a section heading** (no orphan body text or images before first H2).

### Metadata Validation Rules

Every Learning Path `_index.md` requires these YAML fields:
- `description`: One sentence describing task + technology + outcome. Used in search snippets.
- `skilllevels`: Single value - either "Introductory" or "Advanced" only
- `subjects`: Single value from allowlist matching the directory category
- `weight`: 1 for `_index.md`, then 2+ for additional pages in order
- `layout`: "learningpathall" for Learning Paths; "installtoolsall" for install guides

Install guides require:
- `weight: 1` (always)
- `layout: installtoolsall`
- `tool_install: true` (except when hidden)
- `official_docs`: Link to official documentation
- `minutes_to_complete`: Realistic time estimate
...
### Validation Automation First
Always run validation before suggesting content completion:
```bash
python3 tools/verify_index_fields.py      # Fails on missing required YAML fields or mismatched subjects/category
python3 tools/style_check.py <filepath>   # Validates writing style (passive voice, word choices, contractions)
```
Fix validation errors immediately—they block publication. The `verify_index_fields.py` enforces that:
- `skilllevels` must be exactly "Introductory" or "Advanced" (no variations)
- `subjects` values are category-specific and checked against `closed-filters-allow-list.yml`
- `_index.md` files must include `description` field (used for search snippets, not optional)

### File Weight Ordering Pattern
- `_index.md` always has `weight: 1`
- Additional pages in sequence: `weight: 2`, `weight: 3`, etc.
- No duplicate weights in the same directory
- Learning Paths require both `_index.md` AND `_next-steps.md` (use template from `archetypes/`)

### Metadata Description Pattern (Not Title Repetition)
The `description` field is critical for AI discoverability. Structure as:
- **Bad**: "Describes how to use Linux" (generic, repeats title)
- **Good**: "Learn how to profile C++ applications on Arm Neoverse using Performix to identify memory bottlenecks and improve throughput by 25%"

Include: verb + task + platform/tool + measurable outcome

### Testing Embedded Code Blocks
When Learning Paths include executable code, add test metadata:
```bash
```bash
# This test checks setup works correctly
# Test metadata: cmd=command, expect_output="text_to_find"
your_command_here
```
The `check.py` tool parses these and runs them in Docker containers, outputting JUnit reports for CI/CD.

### Content Scope Boundaries
**Learning Paths = end-to-end workflows**: prepare → configure → use → validate  
**Install guides = setup only**: installation, configuration, verification (no usage workflows)

Never duplicate install steps inside a Learning Path. Link to install guides instead.
